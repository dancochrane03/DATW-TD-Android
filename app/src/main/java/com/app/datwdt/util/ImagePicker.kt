...
class ImagePicker : DialogFragment(), View.OnClickListener {
    ...
    private fun chkAllPermission(requestCode: Int): Boolean {
        ...
        for (p in permissions) {
            result = ContextCompat.checkSelfPermission(DatwdtApplication.context, p)
    ...
    private fun callCamera() {
        ...
        if (takePictureIntent.resolveActivity(requireActivity().packageManager) != null) {
            ...
            try {
                ...
                LogUtils.Print(TAG, "Create Image File")
            } catch (ex: IOException) {
                LogUtils.Print(TAG, "Error File$ex")
            }
    ...
    private fun createImageFile(): File {
        ...
        val storageDir =
            DatwdtApplication.context.getExternalFilesDir(Environment.DIRECTORY_PICTURES)
        ...
        if (resultCode == Activity.RESULT_OK) {
            ...
            if (requestCode == RESULT_LOAD_IMAGE) {
                if (data != null) {
                    uri = data.data
                    if (uri != null) {
                        LogUtils.Print("uriname=>", "" + uri)
                        imgPath = getRealPathFromUri(DatwdtApplication.context, uri)
                        LogUtils.Print(TAG, "Gallery Path: $imgPath")
                        if (listener != null) {
                            listener!!.set(imgPath!!)
                        }
                        dismiss()
                    } else {
                        LogUtils.Print(TAG, "uri null: ")
                    }
                }
            } else if (requestCode == CAMERA_REQUEST) {
                LogUtils.Print("CAMERA_REQUEST", "load 1")
                val cr = DatwdtApplication.context.contentResolver
                try {
                    val bitmap = MediaStore.Images.Media.getBitmap(cr, imageUri)
                    uri = getImageUri(DatwdtApplication.context, bitmap)
                    if (uri != null) {
                        imgPath = getRealPathFromUri(DatwdtApplication.context, uri)
                        LogUtils.Print(TAG, "Camera Path: $imgPath")
                        if (listener != null) {
                            listener!!.set(imgPath!!)
                        }
                        dismiss()
                    ...
                    LogUtils.Print("CAMERA_REQUEST", "Failed to load" + e.message)
                }
            }
        } else {
            dismiss()
